...
# Mandatory Functions
- `update!(sampler, θ, ∇θ)` where θ the current parameter vector and ∇θ(θ) is a
...
update!(sampler::MCMCState, θ::AbstractVector{T}, ∇θ) where {T} = error("$(typeof(sampler)) has not implemented an update! method. Please consult the documentation for MCMCState")
